...
public class TestUserActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    updateButton.setOnClickListener(v -> fetchFirebaseData(uuidText.getText().toString()));

    // Get uuid stored in local data
    loadButton.setOnClickListener((OnClickListener) v -> {
...
//        uuidText.setText(createdUser.getUuid());
  ...
  private void fetchFirebaseData(String docName) {
    firestore.collection("users")
        .document(docName).get().addOnCompleteListener(task -> {
